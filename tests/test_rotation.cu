...
int main() {
    std::cout << "=== Test Rotation Filter ===" << std::endl;
    
    ImageData input, output90;
    ...
    std::cout << "Testing rotate90..." << std::endl;
    if (!RotationFilter::rotate90(input, output90)) {
        std::cerr << "Rotate90 failed" << std::endl;
        return 1;
    }
    
    if (!processor.copyDeviceToHost(output90, output90)) {
    ...
    std::cout << "Output: " << output90.width << "x" << output90.height << "x" << output90.channels << std::endl;
